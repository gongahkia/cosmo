-- Coral reef generator using L-systems and boid flocking
-- Research: L-systems for coral growth and Reynolds boids for fish schools

local math_utils = require("helper.math")

local coral = {}

function coral.generate(width, height, params)
    params = params or {}
    ...
    for _ = 1, 50 do
        for _, fish in ipairs(fish_schools) do
            ...
            fish.x = math_utils.clamp(fish.x + fish.dx, 1, width)
            fish.y = math_utils.clamp(fish.y + fish.dy, 1, height * 0.6)
